...
mkdir /webdav /ocx /dl
apt install -y mosh links tasksel make cmake build-essential unzip gcc
...
echo alias nsite='echo SITES-ENABLED && ls /etc/nginx/sites-enabled/ && echo SITES-AVAILABLE && ls /etc/nginx/sites-available' >> ~/.bashrc
